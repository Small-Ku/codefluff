...
function getModelConfig(modelArg?: string): ModelConfig {
  ...
  if (!config) {
    console.error(
      `❌ Unknown model: "${key}". Available models: ${Object.keys(MODEL_CONFIGS).join(', ')}`,
    )
    process.exit(1)
  }
  return config
...
const modelArg = process.argv.find(
  (a, i) => i > 1 && !a.startsWith('-') && a !== 'long',
)
...
async function makeConversationStreamRequest(
  ...
  if (!response.ok) {
    ...
    console.error(
      `❌ Fireworks streaming API returned ${response.status}: ${errorText}`,
    )
    return {
      label,
      usage: null,
  ...
  console.log(
    `   ✅ ${(elapsedMs / 1000).toFixed(2)}s | TTFT ${ttftMs !== undefined ? (ttftMs / 1000).toFixed(2) + 's' : 'n/a'} | ${inputTokens} in (${cachedTokens} cached, ${cacheRate}%) | ${outputTokens} out @ ${outputTokensPerSec !== undefined ? outputTokensPerSec.toFixed(1) + ' tok/s' : 'n/a'} | ${cost}`,
  )
  console.log(
