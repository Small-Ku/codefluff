/**
 * E2E test that verifies Codefluff can run terminal commands.
 *
 * Starts Codefluff in tmux, sends a prompt asking it to run a shell command,
...
describe('Codefluff: Terminal Command', () => {
  ...
  test(
    ...
    async () => {
      ...
      session = await CodefluffSession.start(binary, { waitSeconds: 5 })

      // Wait for the CLI to be fully ready before sending input
      await session.waitForReady()

      // Ask codefluff to run a shell command whose output can only come from
