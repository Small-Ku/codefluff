...
export class CodefluffSession {
  ...
  /**
   * Start a codefluff binary in a tmux session.
  ...
  static async start(
  ...
  ): Promise<CodefluffSession> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codefluff-e2e-'))

    // Create a minimal project so codefluff has something to work with
    fs.writeFileSync(
    ...
    return new CodefluffSession(sessionName, tmpDir)
  }
  ...
  /** Send text input to the codefluff CLI (presses Enter by default). */
  async send(
    text: string,
