...
-  **Package Management:** When adding new packages, use the ${PLACEHOLDER.SHELL_AGENT_PROMPT} agent to install the package rather than editing the package.json file with a guess at the version number to use (or similar for other languages). This way, you will be sure to have the latest version of the package. Do not install packages globally unless asked by the user (e.g. Don't run \`npm install -g <package-name>\`). Always try to use the package manager associated with the project (e.g. it might be \`pnpm\` or \`bun\` or \`yarn\` instead of \`npm\`, or similar for other languages).
...
- **Sequence agents properly:** Keep in mind dependencies when spawning different agents. Don't spawn agents in parallel that depend on each other.
  ${buildArray(
        ...
        `- Spawn ${PLACEHOLDER.SHELL_AGENT_PROMPT} sequentially if the second command depends on the the first.`,
        isDefault &&
...
${isDefault
        ? `[ You spawn a code-reviewer, ${PLACEHOLDER.SHELL_AGENT_PROMPT} to typecheck the changes, and another ${PLACEHOLDER.SHELL_AGENT_PROMPT} to run tests, all in parallel ]`
        : isFree
          ? `[ You spawn a code-reviewer-lite to review the changes, ${PLACEHOLDER.SHELL_AGENT_PROMPT} to typecheck the changes, and another ${PLACEHOLDER.SHELL_AGENT_PROMPT} to run tests, all in parallel ]`
          : isMax
            ? `[ You spawn ${PLACEHOLDER.SHELL_AGENT_PROMPT} to typecheck the changes, and another ${PLACEHOLDER.SHELL_AGENT_PROMPT} to run tests, in parallel. Then, you spawn a code-reviewer-multi-prompt to review the changes. ]`
            : `[ You spawn ${PLACEHOLDER.SHELL_AGENT_PROMPT} to typecheck the changes and another ${PLACEHOLDER.SHELL_AGENT_PROMPT} to run tests, all in parallel ]`
      }
...
${isDefault
        ...
        : isFree
          ...
          : isMax
            ...
            : `[ You fix the issues found by the type/test errors and spawn more ${PLACEHOLDER.SHELL_AGENT_PROMPT} to confirm ]`
      }
...
const EXPLORE_PROMPT = `- Iteratively spawn file pickers, ${PLACEHOLDER.SHELL_AGENT_PROMPT}, and web/docs researchers to gather context as needed. Use the code_search, list_directory, and glob tools directly for searching and exploring the codebase. The file-picker agent in particular is very useful to find relevant files -- try spawning multiple in parallel (say, 2-5) to explore different parts of the codebase. Use read_subtree if you need to grok a particular part of the codebase. Read all the relevant files using the read_files tool.`
