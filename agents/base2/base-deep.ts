...
- **Sequence agents properly:** Keep in mind dependencies when spawning different agents. Don't spawn agents in parallel that depend on each other.
  ...
  - Spawn ${PLACEHOLDER.SHELL_AGENT_PROMPT} agents sequentially if the second command depends on the the first.
...
1. Run any existing unit tests that cover the modified code (spawn ${PLACEHOLDER.SHELL_AGENT_PROMPT} agents in parallel for typechecks, tests, lints as appropriate).
